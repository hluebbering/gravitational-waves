...
import numpy as np
from legwork import evol, strain, psd
...
def snr_circ_stationary(m_c, f_orb, dist, t_obs, position=None, polarisation=None, inclination=None,
    ...
    dist (float/array): Distance to the source
    t_obs (float): Total duration of the observation
    ...
    interpolated_sc (function): Computes the LISA sensitivity curve.
    instrument : `{{ 'LISA', 'TianQin', 'custom' }}`
    custom_psd (function): Computing the PSD

    Returns:
    snr (float/array): SNR for each binary
    """
...
def snr_ecc_stationary(m_c, f_orb, ecc, dist, t_obs, harmonics_required,
    ...
    """
    Parameters:
    ecc (float/array): Eccentricity
    harmonics_required (integer): Maximum integer harmonic to compute
    ret_max_snr_harmonic (boolean): Whether to return the harmonic with the maximum SNR
    ret_snr2_by_harmonic (boolean): Whether to return the SNR^2 in each individual harmonic
    """
...
def snr_circ_evolving(m_1, m_2, f_orb_i, dist, t_obs, n_step,
    ...
    f_orb_i : `float/array` Initial orbital frequency
    n_step : `int` Number of time steps during observation duration
    t_merge : `float/array` Time until merger
    """
...
def snr_ecc_evolving(m_1, m_2, f_orb_i, dist, ecc, harmonics_required, t_obs, n_step,
    ...
    """
    Parameters
    n_proc : `int` Number of processors to split eccentricity evolution over
    """
    m_c = utils.chirp_mass(m_1=m_1, m_2=m_2)
